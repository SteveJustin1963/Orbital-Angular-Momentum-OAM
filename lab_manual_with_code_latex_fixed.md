...
> **LaTeX compatibility note:** This version avoids the disallowed operator-name macro inside equations. Function names such as atan2 and sgn are written with roman math styling so stricter Markdown/LaTeX renderers will not reject the formulas.
...
\[
I(\theta)=I_{\mathrm{leak}}+I_0\cos^2\theta
\]
...
\[
E_{\mathrm{annulus}}=E_{\mathrm{outer}}-E_{\mathrm{inner}}
\]
...
For \(P=5\,\mathrm{mW}\), \(l=1\), and \(\lambda=532\,\mathrm{nm}\):

\[
\omega\approx3.54\times10^{15}\,\mathrm{rad/s}
\]

\[
\tau\approx\frac{5\times10^{-3}}{3.54\times10^{15}}
\approx1.4\times10^{-18}\,\mathrm{N\,m}
\]
...
If a rotating object or optical element changes angular momentum from \(l_{\mathrm{in}}\) to \(l_{\mathrm{out}}\), the optical frequency shift is:

\[
\Delta \omega = (l_{\mathrm{out}}-l_{\mathrm{in}})\Omega
\]

or:

\[
\Delta f = \frac{(l_{\mathrm{out}}-l_{\mathrm{in}})\Omega}{2\pi}
\]
...
Appendix A in this same file contains the complete starter code for:
